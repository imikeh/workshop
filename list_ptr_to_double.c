...
double del( NodePtr *start, NodePtr *current ) // delete function
{
	...
	double value = *(temp->data); // hold the return value
...
void add( NodePtr *start, NodePtr *current, double input ) // add function
{
	if( *current == NULL ) // if the current Node is NULL
	{
		*start = (NodePtr) my_malloc( sizeof( Node ) ); // make a new Node in dlist
		...
		// set the start Node
		(*start)->data = (double *) my_malloc( sizeof( double ) );
		*( (*start)->data ) = input;
		(*start)->right = NULL;
		(*start)->left = NULL;
	...
	else
	{
		...
		// set the new Node
		newptr->data = (double *) my_malloc( sizeof( double ) );
		*(newptr->data) = input;
		newptr->right = NULL;
		newptr->left = NULL;
