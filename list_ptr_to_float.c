...
float del( NodePtr *start, NodePtr *current ) // delete function
{
	...
	float value = *(temp->data); // hold the return value
...
void add( NodePtr *start, NodePtr *current, float input ) // add function
{
	if( *current == NULL ) // if the current Node is NULL
	{
		*start = (NodePtr) my_malloc( sizeof( Node ) ); // make a new Node in dlist
		...
		// set the start Node
		(*start)->data = (float *) my_malloc( sizeof( float ) );
		*((*start)->data) = input;
		(*start)->right = NULL;
		(*start)->left = NULL;
	...
	else
	{
		...
		// set the new Node
		newptr->data = (float *) my_malloc( sizeof(float) );
		*(newptr->data) = input;
		newptr->right = NULL;
		newptr->left = NULL;
