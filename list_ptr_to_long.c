...
long del( NodePtr *start, NodePtr *current ) // delete function
{
	...
	long value = *(temp->data); // hold the return value
...
void add( NodePtr *start, NodePtr *current, long input ) // add function
{
	if( *current == NULL ) // if the current Node is NULL
	{
		*start = (NodePtr) my_malloc( sizeof( Node ) ); // make a new Node in dlist
		...
		// set the start Node
		(*start)->data = (long *) my_malloc( sizeof( long ) );
		*((*start)->data) = input;
		(*start)->right = NULL;
		(*start)->left = NULL;
	...
	else
	{
		...
		// set the new Node
		newptr->data = (long *) my_malloc( sizeof( long ) );
		*(newptr->data) = input;
		newptr->right = NULL;
		newptr->left = NULL;
