...
short del( NodePtr *start, NodePtr *current ) // delete function
{
	...
	short value = *(temp->data); // hold the return value
...
void add( NodePtr *start, NodePtr *current, short input ) // add function
{
	if( *current == NULL ) // if the current Node is NULL
	{
		*start = (NodePtr) my_malloc( sizeof( Node ) ); // make a new Node in dlist
		...
		// set the start Node
		(*start)->data = (short *) my_malloc( sizeof( short ) );
		*((*start)->data) = input;
		(*start)->right = NULL;
		(*start)->left = NULL;
	...
	else
	{
		...
		// set the new Node
		newptr->data = (short *) my_malloc( sizeof( short ) );
		*(newptr->data) = input;
		newptr->right = NULL;
		newptr->left = NULL;
