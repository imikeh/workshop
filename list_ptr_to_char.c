...
char del( NodePtr *start, NodePtr *current ) // delete function
{
	...
	char value = *(temp->data); // hold the return value
...
void add( NodePtr *start, NodePtr *current, char input ) // add function
{
	if( *current == NULL ) // if the current Node is NULL
	{
		*start = (NodePtr) my_malloc( sizeof( Node ) ); // make a new Node in dlist
		...
		// set the start Node
		(*start)->data = (char*) my_malloc( sizeof( char ) );
		*((*start)->data) = input;
		(*start)->right = NULL;
		(*start)->left = NULL;
	...
	else
	{
		...
		// set the new Node
		newptr->data = (char*) my_malloc( sizeof( char ) );
		*(newptr->data) = input;
		newptr->right = NULL;
		newptr->left = NULL;
